...
Market selection is before today's Asia/Taipei calendar date, using the latest available completed trading session. Weekends and market holidays continue to display the most recent completed trading session; the actual source trading date is always displayed and is never relabelled as yesterday. A snapshot crossing Taipei midnight is not, by itself, an update failure and must not trigger the stale/refresh-pending warning. That warning is reserved for an actual source error or use of fallback/cached market data. US bars from the current New York day are excluded until 16:15 New York time. Quotes are reference closes, not real-time trading quotes. FX is a daily reference rate, not a bank cash/spot buy/sell quote.
