...
const {day,weatherInfo,validWeather,marketStale}=require('../assets/js/press.js');
...
test('Midnight rollover alone does not mark market data stale',()=>{assert.equal(marketStale({date:'2026-09-23',errors:{}},'tw'),false);assert.equal(marketStale({date:'2026-09-23',errors:{tw:'source failed'}},'tw'),true);assert.equal(marketStale({date:'2026-09-23',errors:{},cached:true},'tw'),true);});
